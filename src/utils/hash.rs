...
/// Argon2id parameters: t=2, m=19456 (19MB), p=1
//
// // Argon2id 参数：t=2, m=19456 (19MB), p=1
pub const ARGON2_PARAMS: Params = match Params::new(19456, 2, 1, Some(32)) {
    Ok(p) => p,
...
pub fn hash_password(password: &str) -> Result<String> {
    // 1. 生成随机盐值（Argon2 库会自动处理 PHC 格式中的盐）。
    let salt = SaltString::generate(&mut OsRng);

    // 2. 使用明确定义的参数初始化 Argon2id。
