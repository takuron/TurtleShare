...
cors_origins = ["https://admin.example.com"]  # Extra allowed CORS origins; same-origin is always allowed / 额外允许的 CORS 来源；同源请求始终允许
...
- `cors_origins`: Additional allowed CORS origins as an array of origin strings such as `https://admin.example.com`. Same-origin requests derived from `base_url` are always allowed, and an empty array means only same-origin requests are permitted. / 额外允许的 CORS 来源列表，使用诸如 `https://admin.example.com` 的来源字符串数组。同源请求会根据 `base_url` 自动允许；若该数组为空，则仅允许同源请求。
